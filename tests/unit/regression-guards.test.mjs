...
const stateStorage = readFileSync('app-state-storage-v1.js', 'utf8');
const cloudAuth = readFileSync('cloud-auth-v5.js', 'utf8');
const radar = readFileSync('radar-20.js', 'utf8');
const detail = readFileSync('detail-page-unified-v1.js', 'utf8');
...
assert.match(cloudAuth, /upsertLocal\(data,\{ silent=false,userId=currentUser\?\.id,context=null \}=\{\}\)/);
assert.match(cloudAuth, /commitSyncedState\(userId,data,now,uploadContext\)/);
