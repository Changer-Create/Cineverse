(() => {
  ...
  function loadSupabase() {
    ...
    return new Promise((resolve,reject) => {
      ...
      // Keep third-party executable code on a reviewed, immutable release rather
      // than silently accepting every future major-compatible CDN update.
  ...
  function renderSignedIn() {
    ...
    body.innerHTML = `<div class="movie-account-card"><div class="movie-account-email">${escapeHtml(currentUser.email)}</div><div class="movie-account-syncstate" id="movieAccountSyncState">${escapeHtml(stateText)}</div><div class="movie-account-actions ${pendingConflict?'has-conflict':''}">${actions}</div></div><div class="movie-cloud-note">${note}</div>`;
  }
  function openDialog() {
